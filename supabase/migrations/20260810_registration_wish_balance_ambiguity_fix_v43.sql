-- Ancient Pulls V43
-- Repairs complete_player_registration() after the tester signup promotion.
--
-- The function returns a column named wish_balance. V38 also used an
-- unqualified wish_balance reference inside an UPDATE, so PL/pgSQL could not
-- decide whether it meant the output variable or player_wallets column.
-- PostgreSQL raised 42702 and rolled back the new profile and wallet.
--
-- Safe to run repeatedly. Existing accounts and promotion grants are kept.
...
begin
  ...
  if to_regclass('public.player_promotions') is null
     or to_regclass('public.player_promotion_grants') is null then
    raise exception 'The tester promotion tables are missing. Run V38 first.';
  end if;
end;
$preflight$;
...
begin
  ...
  if found then
    ...
    insert into public.player_promotion_grants as promotion_grant (
      user_id,
      promotion_key,
      wishes,
      granted_at
    ...
    if coalesce(v_awarded_wishes, 0) > 0 then
      ...
      set
        wish_balance = greatest(coalesce(wallet.wish_balance, 0), 0)
          + v_awarded_wishes,
        updated_at = now()
