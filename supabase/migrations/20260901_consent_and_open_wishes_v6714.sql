-- Ancient Pulls V67.14
-- Repairs the purchase-consent column collision and retires the obsolete
-- pre-release registration gate from ordinary, authenticated wishes.
--
...
create index if not exists player_wish_requests_created_idx
  on public.player_wish_requests (created_at);

alter table public.player_wish_requests enable row level security;
revoke all on table public.player_wish_requests from public, anon, authenticated;
grant all on table public.player_wish_requests to service_role;
...
declare
  v_user_id uuid := auth.uid();
  v_existing public.player_wish_requests%rowtype;
...
begin
  ...
  select request_row.*
  into v_existing
  from public.player_wish_requests as request_row
  ...
  insert into public.player_wish_requests (
    user_id,
    idempotency_key,
    wish_id,
    card_id,
...
comment on function public.make_player_wish(uuid) is
  'V67.14 public wish engine: authenticated, idempotent, server-authoritative, and independent of pre-release registration lists.';
