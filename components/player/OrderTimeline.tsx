...
export default function OrderTimeline({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "w-full" : "mx-auto w-full max-w-[1180px] px-4 py-7 sm:px-6 lg:px-8 lg:py-10"}>
      {!embedded ? <PlayerPageHeader eyebrow="Fulfilment" title="Cards & Orders" description="Track physical cards and deliveries." /> : null}

      {error ? <div className="mt-4"><PlayerErrorBanner message={error} /></div> : null}

      <div className={(embedded ? "" : "mt-5 ") + "grid grid-cols-2 gap-3 lg:grid-cols-4"}>
      ...
      <PlayerPanel className="mt-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">Fulfilment timeline</h2><p className="mt-1 text-xs font-semibold text-white/38">Every card, shipment, and delivery in one place.</p></div>{!embedded ? <div className="flex gap-2"><Link href="/shipping" className="rounded-xl border border-cyan-100/15 bg-cyan-100/[.05] px-3 py-2 text-xs font-black text-cyan-50/70">Shipping</Link><Link href="/help#support" className="rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-xs font-black text-white/60">Get help</Link></div> : null}</div>
