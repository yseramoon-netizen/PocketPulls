...
export default function ConstellationArtwork({sign,className=""}:{sign?:string|null;className?:string}){
 ...
 return <svg viewBox="0 0 100 100" fill="none" className={`ap-constellation-art ${className}`} aria-hidden="true">
  ...
  {segments.map(([a,b],i)=><path key={i} className="ap-art-link" pathLength="1" style={{"--delay":`${i*65}ms`} as CSSProperties} d={`M${points[a].x} ${points[a].y}L${points[b].x} ${points[b].y}`} stroke="#d7c397" strokeWidth=".22" strokeOpacity=".66"/>)}
  {points.map((p,i)=><g key={i} className="ap-art-star" style={{"--delay":`${250+i*55}ms`} as CSSProperties}><circle cx={p.x} cy={p.y} r="2.3" fill="#d5bd87" opacity=".08"/><circle cx={p.x} cy={p.y} r="1.3" fill="#c3deee" opacity=".12"/><path d={`M${p.x-1} ${p.y}h2m-1-1v2`} stroke="#f5ebd6" strokeWidth=".22"/><circle cx={p.x} cy={p.y} r=".34" fill="#f4ebd8"/></g>)}
 </svg>;
}
