...
import { ObservatoryRenderer, HOME_VIEW, buildObservatoryScene, clampDistance, dampView, observatoryLevel, type ObservatoryView, type ObservatoryFrame, type ObservatoryScene } from './ObservatoryRenderer';
...
export default function Observatory(){
  ...
  const [level,setLevel]=useState<'stars'|'galaxy'|'universe'>('stars'),[paused,setPaused]=useState(false),[systemReduced,setSystemReduced]=useState(false);
  ...
  const scene=useMemo(()=>user?buildObservatoryScene(stars,zodiac,players,user):null,[stars,zodiac,players,user]);
  ...
  useEffect(()=>{
    ...
    const draw=(stamp:number)=>{
      ...
      const nextLevel=observatoryLevel(current.current.distance);
      ...
      const label=ownerLabel.current;if(label){
        ...
        label.style.visibility=current.current.distance>.44&&point.x>-20&&point.x<width+20&&point.y>-20&&point.y<height+20?'visible':'hidden';
  ...
  const goTo=useCallback((distance:number)=>{
    Object.assign(target.current,{...HOME_VIEW,distance});setSelectedStar(null);setSelectedGalaxy(null);setPanel(null);setSearch('');
  ...
  const zoomBy=useCallback((amount:number)=>{
    ...
    target.current.distance=clampDistance(target.current.distance+amount);
  ...
  const visitGalaxy=useCallback((player:LeaderboardPlayer)=>{
    ...
    Object.assign(target.current,{distance:1,focusX:position.x*.64,focusY:position.y*.64,focusZ:position.z*.64,starX:0,starY:0,starZ:0});
  ...
  const pointerMove=useCallback((event:ReactPointerEvent<HTMLDivElement>)=>{
    ...
    if(g.points.size>=2){
      ...
      if(a>4&&b>4)zoomBy(-Math.log(b/a)*.48);g.moved=true;return;
    }
  ...
  useEffect(()=>{
    ...
    const wheel=(event:WheelEvent)=>{event.preventDefault();const unit=event.deltaMode===1?16:event.deltaMode===2?element.clientHeight:1;zoomBy(Math.max(-.18,Math.min(.18,event.deltaY*unit*.00052)));};
  ...
  const finishArrival=useCallback(()=>{arriving.current.clear();setArrivalIds([]);clearArrivalQuery();setArrivalMessage('Your new stars are home.');requestFrame.current();},[]);
  ...
  return <section className={`ap-scene ${styles.page}`} data-observatory-level={level}>
    <SceneHeader eyebrow="One continuous sky" title="Observatory" description={level==='stars'?(zodiac?`${ZODIAC_SHAPES[zodiac].label}. Written in your stars.`:'Every card has a place in your sky.'):level==='galaxy'?'A constellation within a galaxy.':'Your place among the galaxies.'}>
    ...
    <div ref={viewport} className={styles.viewport} data-onboarding-target="constellation" tabIndex={0} aria-label="Explore the Observatory" aria-describedby="observatory-controls" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}
      ...
      <button ref={ownerLabel} className={styles.ownerLabel} onClick={()=>goTo(0)} style={{visibility:'hidden'}} aria-label="Enter your constellation"><i/><span>{rank===1?'Your singularity':'Your galaxy'}<small>{rank?`#${rank}${rank===1?' · Pharaoh':''}`:'Personal galaxy'} · Enter</small></span></button>
    </div>
    <div className={styles.vignette} aria-hidden="true"/>
    {(loading||(!painted&&!error))&&<div className={styles.loading} role="status"><span/><p>Opening your sky</p></div>}
    {error&&<div className={styles.error} role="alert"><p>{error}</p><button onClick={()=>void load()}>Try again</button></div>}
    {!loading&&scene&&stars.length===0&&level==='stars'&&!error&&<div className={styles.empty}><AstralIcon/><h2>A sky waiting for its first star.</h2><p>Make a wish, or zoom out to explore the universe.</p><Link href="/wishes">Make a wish <AstralIcon name="arrow"/></Link></div>}
    {panel&&<aside ref={panelRef} className={`ap-scene-panel ${styles.panel}`} role="dialog" aria-modal="true" aria-label={panel==='cards'?'Find a card':panel==='galaxies'?'Find a galaxy':'Observatory information'} tabIndex={-1}>
      ...
      {panel!=='info'?<>
        ...
        <p>The Pharaoh is the central black hole. Larger collections occupy larger galaxies, closer to the centre.</p>
    ...
    {selectedGalaxy&&!panel&&<aside className={`ap-scene-panel ${styles.memory}`} aria-label="Galaxy details"><div className="ap-panel-heading"><div><p>{selectedGalaxy.rank===1?'#1 · The Pharaoh':selectedGalaxy.rank?`Galaxy #${selectedGalaxy.rank}`:'Your personal galaxy'}</p><h2>{selectedGalaxy.displayName}</h2></div><button className="ap-scene-button" onClick={()=>setSelectedGalaxy(null)} aria-label="Close galaxy"><AstralIcon name="close"/></button></div>{selectedGalaxy.rank>0?<div className={styles.stats}><Stat label="Cards" value={selectedGalaxy.totalCards.toLocaleString('en-GB')}/><Stat label="Unique cards" value={selectedGalaxy.uniqueCards.toLocaleString('en-GB')}/><Stat label="Collection value" value={formatMoney(selectedGalaxy.collectionValue)}/><Stat label="Wishes" value={selectedGalaxy.lifetimeWishes.toLocaleString('en-GB')}/></div>:<p>Your galaxy is shown outside the ranked top 100.</p>}{selectedGalaxy.isCurrentUser&&<button className={styles.personal} onClick={()=>goTo(0)}>Enter my constellation <AstralIcon name="arrow"/></button>}</aside>}
    ...
    {!loading&&!panel&&<div className={styles.controls}>
      <nav className={styles.stages} aria-label="Observatory scale"><button aria-current={level==='stars'?'step':undefined} onClick={()=>goTo(0)}>My stars</button><span/><button aria-current={level==='galaxy'?'step':undefined} onClick={()=>goTo(.54)}>My galaxy</button><span/><button aria-current={level==='universe'?'step':undefined} onClick={()=>goTo(1)}>Universe</button></nav>
      <div className={styles.dock}><button aria-label="Zoom out" onClick={()=>zoomBy(.13)}>−</button><span className={styles.scaleLabel}>{level==='stars'?'Constellation':level==='galaxy'?'Galaxy':'Universe'}</span><button aria-label="Zoom in" onClick={()=>zoomBy(-.13)}>+</button><i/>{level==='universe'&&!reduced&&<button onClick={()=>setPaused(value=>!value)} aria-pressed={paused}>{paused?'Play':'Pause'}</button>}<button aria-label="Return to my stars" onClick={()=>goTo(0)}><AstralIcon name="reset"/></button></div>
      <p>{level==='stars'?'Scroll out to discover your place in the universe.':level==='galaxy'?'Keep zooming out. There is a universe around you.':'Your galaxy is marked. Zoom back in to come home.'}</p>
    </div>}
