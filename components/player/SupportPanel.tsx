...
export default function SupportPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "w-full" : "mx-auto w-full max-w-[1180px] px-4 py-7 sm:px-6 lg:px-8 lg:py-10"}>
      {!embedded ? <PlayerPageHeader eyebrow="Player care" title="Support" description="Report a problem and follow every reply." /> : null}
        ...
        <PlayerPanel className="min-h-[24rem] p-4 sm:p-5 lg:p-6">
          {creating ? (
            <form onSubmit={createTicket} className="space-y-4">
              ...
              <label className="block text-xs font-black text-white/60">Photo (optional)<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setPhoto(event.target.files?.[0] || null)} className="mt-1.5 block min-h-12 w-full overflow-hidden rounded-xl border border-white/10 bg-black/20 text-xs text-white/45 file:mr-3 file:min-h-12 file:border-0 file:border-r file:border-white/10 file:bg-white/[.07] file:px-3 file:text-xs file:font-black file:text-white/75" /></label>
            ...
            <div>
              ...
              {selected.status !== "closed" ? <form onSubmit={sendReply} className="mt-4 space-y-3"><textarea required maxLength={4000} rows={4} value={reply} onChange={(event) => setReply(event.target.value)} placeholder="Add a reply…" className="w-full rounded-xl border border-white/10 bg-black/20 p-3 text-base text-white outline-none focus:border-cyan-100/30 sm:text-sm" /><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setPhoto(event.target.files?.[0] || null)} className="block min-h-12 w-full overflow-hidden rounded-xl border border-white/10 bg-black/20 text-xs text-white/45 file:mr-3 file:min-h-12 file:border-0 file:border-r file:border-white/10 file:bg-white/[.07] file:px-3 file:text-xs file:font-black file:text-white/75" /><PlayerPrimaryButton type="submit" disabled={busy || !reply.trim()}>{busy ? "Sending…" : "Send reply"}</PlayerPrimaryButton></form> : null}
            </div>
