...
/* eslint-disable @next/next/no-img-element -- card art is served by catalogue hosts */
...
export default function ShippingCentre({
  embedded = false,
}: {
  embedded?: boolean;
}) {
  ...
  const [shipments, setShipments] =
    useState<Shipment[]>([]);
  const [shippableCards, setShippableCards] =
    useState<ShippableCardRow[]>([]);
  const [selectedWishIds, setSelectedWishIds] =
    ...
    try {
      const [
        ...
        supabase
          ...
          .limit(30),
        supabase.rpc("get_player_order_timeline", {
          p_limit: 250,
        }),
      ]);
      ...
      const nextShippableCards = Array.isArray(cardsResult.data)
        ? (cardsResult.data as ShippableCardRow[]).filter(
            (card) =>
              !card.shipment_id &&
              ["ready", "sourced"].includes(card.fulfilment_status),
          )
        : [];

      setShippableCards(nextShippableCards);
      setSelectedWishIds((current) => {
        const availableIds = new Set(
          nextShippableCards.map((card) => card.wish_id),
        );
        return new Set(
          Array.from(current).filter((wishId) => availableIds.has(wishId)),
        );
      });
  ...
  const filteredShippableCards = useMemo(() => {
    const query = cardSearch.trim().toLowerCase();
    if (!query) return shippableCards;

    return shippableCards.filter((card) =>
      [card.card_name, card.set_name, card.card_no || "", card.rarity]
        .join(" ")
        .toLowerCase()
        .includes(query),
    );
  }, [cardSearch, shippableCards]);

  const selectedCount = selectedWishIds.size;
  const selectedCardsReady =
    selectedCount >= eligibility.threshold;

  const toggleCard = useCallback((wishId: string) => {
    setSelectedWishIds((current) => {
    ...
    try {
      const { error } = await supabase.rpc(
        "request_player_shipment",
        {
          ...
          p_wish_ids: Array.from(selectedWishIds),
        },
      );
  ...
  return (
    <section className={embedded ? "w-full" : "mx-auto w-full max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8"}>
      {embedded ? (
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-100/45">
              From your constellation to your door
            </p>
            <h2 className="mt-2 text-3xl font-black text-white">Choose cards to ship</h2>
            <p className="mt-2 text-sm font-semibold text-white/40">
              Select the exact cards for this parcel, then choose where they should go.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <PlayerSecondaryButton onClick={openNewAddress}>Add address</PlayerSecondaryButton>
            <PlayerSecondaryButton onClick={() => void loadShipping()}>Refresh</PlayerSecondaryButton>
          </div>
        </div>
      ) : (
        <PlayerPageHeader
          ...
          actions={
            <>
              <PlayerSecondaryButton onClick={openNewAddress}>Add address</PlayerSecondaryButton>
              <PlayerSecondaryButton onClick={() => void loadShipping()}>Refresh shipping</PlayerSecondaryButton>
            </>
          }
        />
      )}
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-7 text-white/40">
              {eligibility.activeShipmentId
                ...
                : eligibility.unlocked
                  ? "Choose the cards you want in this parcel, then select a delivery address."
      ...
      <PlayerPanel className="mt-6 overflow-hidden">
        <div className="flex flex-col gap-4 border-b border-white/10 p-5 sm:flex-row sm:items-end sm:justify-between sm:p-6">
          <div>
            ...
            <p className="mt-2 text-sm font-semibold text-white/36">
              {formatWholeNumber(selectedCount)} selected · at least {formatWholeNumber(eligibility.threshold)} required for free shipping
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <PlayerSecondaryButton
              onClick={() => setSelectedWishIds(new Set(shippableCards.map((card) => card.wish_id)))}
              disabled={!shippableCards.length || Boolean(eligibility.activeShipmentId)}
            >
              Select all
            </PlayerSecondaryButton>
            <PlayerSecondaryButton
              onClick={() => setSelectedWishIds(new Set())}
              disabled={!selectedCount || Boolean(eligibility.activeShipmentId)}
            >
              Clear
            </PlayerSecondaryButton>
          </div>
        ...
        <div className="p-4 sm:p-5">
          <input
            ...
            placeholder="Search your shippable cards..."
            className="shipping-input w-full"
          />

          {loading ? (
            <div className="mt-4 h-48 animate-pulse rounded-2xl bg-white/[0.025]" />
          ) : !shippableCards.length ? (
            <div className="mt-4 rounded-2xl border border-white/10 bg-black/15 p-7 text-center text-sm font-semibold text-white/35">
              No cards are currently ready for a new shipment.
            </div>
          ) : (
            <div className="mt-4 grid max-h-[36rem] gap-3 overflow-y-auto pr-1 sm:grid-cols-2 lg:grid-cols-3">
              {filteredShippableCards.map((card) => {
                const selected = selectedWishIds.has(card.wish_id);
                return (
                  <button
                    key={card.wish_id}
                    type="button"
                    onClick={() => toggleCard(card.wish_id)}
                    disabled={Boolean(eligibility.activeShipmentId)}
                    aria-pressed={selected}
                    className={[
                      "grid grid-cols-[3.2rem_minmax(0,1fr)_1.7rem] items-center gap-3 rounded-2xl border p-3 text-left transition disabled:cursor-not-allowed disabled:opacity-50",
                      selected
                        ? "border-cyan-100/35 bg-cyan-200/[0.11] shadow-[0_0_28px_rgba(103,232,249,0.08)]"
                        : "border-white/10 bg-white/[0.035] hover:border-white/20 hover:bg-white/[0.06]",
                    ].join(" ")}
                  >
                    <span className="aspect-[63/88] overflow-hidden rounded-lg bg-white/5">
                      {card.image_url ? (
                        <img src={card.image_url} alt="" loading="lazy" className="h-full w-full object-cover" />
                      ) : null}
                    </span>
                    <span className="min-w-0">
                      <strong className="block truncate text-sm font-black text-white">{card.card_name}</strong>
                      <span className="mt-1 block truncate text-[0.68rem] font-bold text-white/38">
                        {card.set_name}{card.card_no ? ` · #${card.card_no}` : ""}
                      </span>
                      <span className="mt-1 block truncate text-[0.62rem] font-black uppercase tracking-[0.08em] text-cyan-100/46">{card.rarity}</span>
                    </span>
                    <span className={[
                      "flex h-7 w-7 items-center justify-center rounded-full border text-xs font-black",
                      selected ? "border-cyan-100/45 bg-cyan-100 text-cyan-950" : "border-white/15 bg-black/15 text-transparent",
                    ].join(" ")}>
                      ✓
                    </span>
                  </button>
                ...
                {requesting
                  ...
                  : !selectedAddressId
                    ...
                    : !eligibility.unlocked
                      ...
                      : !selectedCardsReady
                        ? `Select ${formatWholeNumber(eligibility.threshold - selectedCount)} more`
