...
export default function TradeRoom({
  ...
  useEffect(() => {
    ...
    const requestedTrade =
      initialTradeId ||
      (tradeFromUrl === "open" ? "" : tradeFromUrl) ||
      "";
  ...
  async function createTrade(
    ...
    try {
      ...
      window.history.replaceState(
        null,
        "",
        `/friends?trade=${encodeURIComponent(
          tradeId,
        )}`,
      );
  ...
  return (
    <section className={embedded ? "w-full px-3 pb-10 pt-3 sm:px-5" : "mx-auto w-full max-w-[1760px] px-4 pb-24 pt-6 sm:px-6 lg:px-8"}>
      <PlayerPageHeader
        eyebrow="Protected card exchange"
        title="Trade Cards"
        description="Exchange cards safely with friends."
        actions={embedded && onClose ? (
          <PlayerSecondaryButton onClick={onClose}>Close trade</PlayerSecondaryButton>
        ) : undefined}
      />
