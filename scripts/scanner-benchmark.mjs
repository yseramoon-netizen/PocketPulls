...
function metrics(records) {
  ...
  const visualMs = records.map((record) => Number(record.timings?.visualMs || 0)).filter(Number.isFinite);
  const candidateMs = records.map((record) => Number(record.timings?.candidateMs || 0)).filter(Number.isFinite);
  return {
    ...
    averageVisualMs: round(mean(visualMs)),
    averageCandidateMs: round(mean(candidateMs)),
    p95VisualMs: round(percentile(visualMs, 0.95)),
    p95OcrMs: round(percentile(ocrMs, 0.95)),
  };
}
...
const result = { overall: metrics(parsed), byStrategy: {}, byTag: {} };
for (const strategy of ["visual-only", "visual-verify", "recovery"]) {
  const records = parsed.filter((record) => record.strategy === strategy);
  if (records.length) result.byStrategy[strategy] = metrics(records);
}
