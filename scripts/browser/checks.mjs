...
async function drag(p,dy,dx=0,touch=false){
 const b=await p.getByTestId('astra-staff').boundingBox(),box=await p.locator('.as-puppet').boundingBox();const scale=box.width/260,x=b.x+b.width/2,y=b.y+b.height/2,r=82*scale,px=box.x+168*scale,py=box.y+163*scale;
 const points=Array.from({length:13},(_,i)=>dy>=100&&!dx?{x:px+Math.sin(i/12*Math.PI/4)*r,y:py-Math.cos(i/12*Math.PI/4)*r}:{x:x+dx*i/12,y:y+dy*i/12});
 if(touch){const c=await p.context().newCDPSession(p);await c.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x,y}]});for(const point of points)await c.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[point]});await c.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await c.detach();}else{await p.mouse.move(x,y);await p.mouse.down();for(const point of points)await p.mouse.move(point.x,point.y);await p.mouse.up();}
}
...
try{
 ...
 await check('Keyboard Right and Enter makes a wish; Escape cancels an armed staff',async()=>{await p.getByTestId('astra-staff').focus();await p.keyboard.press('ArrowRight');await p.keyboard.press('Escape');assert.equal(s.calls.length,2);await p.getByTestId('astra-staff').focus();await p.keyboard.press('ArrowRight');await p.keyboard.press('Enter');await p.getByTestId('wish-result').waitFor();assert.equal(s.calls.length,3);assert.equal(s.balance,0);await p.getByRole('button',{name:'Place in my constellation',exact:false}).click();});
