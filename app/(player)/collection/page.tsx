...
type SortOption = "binder" | "name" | "value_desc" | "value_asc" | "quantity_desc";
...
export default function CollectionPage() {
  ...
  const [sort, setSort] = useState<SortOption>("binder");
  ...
  const loadBinderSettings = useCallback(async () => {
    const { data, error } = await supabase.rpc("get_player_binder_settings");
    if (error) throw error;

    const row = Array.isArray(data) ? data[0] : data;
    if (row && typeof row === "object") {
      const theme = (row as BinderSettingsRow).theme_key;
  ...
  const loadCards = useCallback(
    ...
    [search, setName, rarity, availability, sort, page],
  );
    ...
    <section className={styles.pageShell}>
      ...
      <section className={styles.themeSection}>
        <div className={styles.themeHeading}>
          ...
          <span>Saved to your trainer profile</span>
        </div>
        ...
        <div className={styles.themeGrid}>
          {BINDER_THEMES.map((theme) => (
            <button
              ...
              className={`${styles.themeCard} ${themeKey === theme.key ? styles.themeCardActive : ""}`}
            >
              <span
              ...
              {themeKey === theme.key ? <span className={styles.themeSelected}>Selected</span> : null}
            </button>
          ))}
        </div>
      </section>
