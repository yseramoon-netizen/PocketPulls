...
/* eslint-disable @next/next/no-img-element -- catalogue artwork is hosted remotely */
...
import OrderTimeline from "@/components/player/OrderTimeline";
...
function parseShippingCards(value: unknown): ShippingCard[] {
  if (!Array.isArray(value)) return [];

  return (value as ShippingCardRow[])
    .map((row) => ({
      ...
      rarity: row.rarity?.trim() || "Unknown rarity",
      imageUrl: row.image_url?.trim() || null,
    }))
    .filter((card) => card.cardId && card.availableQuantity > 0);
}
...
export default function ShippingPage() {
  ...
  const [shipments, setShipments] =
    useState<Shipment[]>([]);
  const [shippingCards, setShippingCards] =
    useState<ShippingCard[]>([]);
    ...
    try {
      const [
        ...
        supabase
          ...
          .limit(30),
        supabase.rpc("get_player_shipping_cards"),
      ]);
  ...
  const selectedCardTotal = useMemo(
    () => Object.values(selectedCards).reduce((total, quantity) => total + quantity, 0),
    [selectedCards],
  );

  const visibleShippingCards = useMemo(() => {
    const query = cardSearch.trim().toLowerCase();
    if (!query) return shippingCards;
    return shippingCards.filter((card) =>
      [card.name, card.setName, card.cardNumber, card.rarity]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(query)),
    );
  }, [cardSearch, shippingCards]);

  const changeCardQuantity = useCallback((cardId: string, quantity: number) => {
    const card = shippingCards.find((candidate) => candidate.cardId === cardId);
    if (!card) return;
    const nextQuantity = Math.min(card.availableQuantity, Math.max(0, Math.floor(quantity)));
    setSelectedCards((current) => {
      const next = { ...current };
      if (nextQuantity > 0) next[cardId] = nextQuantity;
    ...
    try {
      const { error } = await supabase.rpc(
        "request_player_shipment",
        {
          ...
          p_card_selection: shippingCards
            .map((card) => ({
              card_id: card.cardId,
              quantity: selectedCards[card.cardId] || 0,
            }))
      ...
      setSuccessMessage(
        formatWholeNumber(selectedCardTotal) + " selected cards are reserved and now in the packing queue.",
      );
      await loadShipping();
    } catch (error: unknown) {
      setErrorMessage(
  ...
  return (
    <section className="mx-auto w-full max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-7 text-white/40">
              {eligibility.activeShipmentId
                ...
                : eligibility.unlocked
                  ? "Choose the exact cards you want, select an address, then request shipment."
        ...
        <PlayerPanel className="mt-6 overflow-hidden">
          <div className="flex flex-col gap-4 border-b border-white/10 p-5 sm:flex-row sm:items-end sm:justify-between sm:p-6">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-white">Build this shipment</h2>
              <p className="mt-2 text-sm font-semibold text-white/38">Select individual cards and quantities. Nothing else will be reserved.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-cyan-100/15 bg-cyan-200/[0.06] px-3 py-2 text-xs font-black text-cyan-50/75">
                {formatWholeNumber(selectedCardTotal)} / {formatWholeNumber(eligibility.threshold)} selected
              </span>
              <button
                type="button"
                onClick={() => setSelectedCards(Object.fromEntries(shippingCards.map((card) => [card.cardId, card.availableQuantity])))}
                className="min-h-10 rounded-xl border border-white/10 bg-white/[0.04] px-3 text-xs font-black text-white/55 transition hover:bg-white/[0.08] hover:text-white"
              >
                Select all
              </button>
              <button
          ...
          <div className="p-4 sm:p-5">
            <label className="relative block">
              <span className="sr-only">Search cards available for shipping</span>
              <input
                value={cardSearch}
                onChange={(event) => setCardSearch(event.target.value)}
                placeholder="Search your available cards…"
                className="min-h-12 w-full rounded-xl border border-white/10 bg-black/20 px-4 pr-11 text-base font-semibold text-white outline-none placeholder:text-white/25 focus:border-cyan-100/25 sm:text-sm"
              />
              <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-white/25" aria-hidden="true">⌕</span>
            </label>

            {loading ? (
              <div className="mt-4 h-64 animate-pulse rounded-2xl bg-white/[0.025]" />
            ) : visibleShippingCards.length === 0 ? (
              <div className="mt-4 rounded-2xl border border-white/8 bg-black/15 px-5 py-12 text-center text-sm font-semibold text-white/35">
                {cardSearch.trim() ? "No available cards match that search." : "No cards are currently available for shipping."}
              </div>
            ) : (
              <div className="mt-4 grid max-h-[34rem] gap-3 overflow-y-auto pr-1 sm:grid-cols-2 xl:grid-cols-3">
                {visibleShippingCards.map((card) => {
                  const selectedQuantity = selectedCards[card.cardId] || 0;
                  return (
                    <article
                      key={card.cardId}
                      className={[
                        "grid grid-cols-[4.3rem_minmax(0,1fr)] gap-3 rounded-2xl border p-3 transition",
                        selectedQuantity > 0
                          ? "border-cyan-100/24 bg-cyan-200/[0.07] shadow-[0_0_2rem_rgba(34,211,238,0.05)]"
                          : "border-white/9 bg-white/[0.025]",
                      ].join(" ")}
                    >
                      <div className="aspect-[63/88] overflow-hidden rounded-xl border border-white/8 bg-white/[0.035]">
                        {card.imageUrl ? <img src={card.imageUrl} alt="" loading="lazy" className="h-full w-full object-cover" /> : null}
                      </div>
                      <div className="flex min-w-0 flex-col">
                        <p className="truncate text-sm font-black text-white/85">{card.name}</p>
                        <p className="mt-1 truncate text-[0.65rem] font-semibold text-white/35">{card.setName} · {card.cardNumber || "No number"}</p>
                        <p className="mt-1 truncate text-[0.58rem] font-black uppercase tracking-[0.1em] text-violet-100/42">{card.rarity}</p>
                        <div className="mt-auto flex items-center justify-between gap-2 pt-3">
                          <span className="text-[0.62rem] font-black text-white/35">{formatWholeNumber(card.availableQuantity)} available</span>
                          <div className="flex items-center rounded-xl border border-white/10 bg-black/20 p-0.5">
                            <button
                              type="button"
                              onClick={() => changeCardQuantity(card.cardId, selectedQuantity - 1)}
                              disabled={selectedQuantity === 0}
                              className="grid h-8 w-8 place-items-center rounded-lg text-sm font-black text-white/55 transition hover:bg-white/10 disabled:opacity-25"
                              aria-label={"Remove one " + card.name}
                            >−</button>
                            <span className="min-w-8 text-center text-xs font-black text-white/80">{selectedQuantity}</span>
                            <button
                              type="button"
                              onClick={() => changeCardQuantity(card.cardId, selectedQuantity + 1)}
                              disabled={selectedQuantity >= card.availableQuantity}
                              className="grid h-8 w-8 place-items-center rounded-lg text-sm font-black text-white/55 transition hover:bg-white/10 disabled:opacity-25"
          ...
          {activeShipment ? (
            ...
            <>
              <p className="mt-4 text-sm font-semibold leading-7 text-white/38">
                ...
                selected cards. Unselected cards stay in your collection.
              </p>
                ...
                {requesting
                  ...
                  : !selectedAddressId
                    ...
                    : !eligibility.unlocked
                      ...
                      : selectedCardTotal < eligibility.threshold
                        ? `${formatWholeNumber(eligibility.threshold - selectedCardTotal)} more to select`
      ...
      <section id="orders" className="mt-6 scroll-mt-24">
        <div className="mb-4">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-violet-100/40">Cards & orders</p>
          <h2 className="mt-2 text-2xl font-black text-white">Everything travelling—or waiting to travel</h2>
          <p className="mt-2 text-sm font-semibold text-white/38">Your physical-card timeline now lives inside Shipping.</p>
        </div>
        <OrderTimeline embedded />
      </section>
