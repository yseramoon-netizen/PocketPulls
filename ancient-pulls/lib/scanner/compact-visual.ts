import { CARD_REGIONS, type CardRegion } from "./regions";
...
function sampleCanvas(
  ...
  width: number,
  height: number,
  region: CardRegion = { x: 0, y: 0, width: 1, height: 1 },
  rotate180 = false,
): Uint8ClampedArray {
...
function oppositeRegion(region: CardRegion): CardRegion {
  return {
...
export function fingerprintCanvas(source: HTMLCanvasElement): CompactVisualFingerprint {
  return createCompactFingerprintFromSamples(
    ...
    sampleCanvas(source, ART_WIDTH, ART_HEIGHT, CARD_REGIONS.artwork),
    sampleCanvas(source, COLOUR_WIDTH, COLOUR_HEIGHT, CARD_REGIONS.artwork),
  );
}
...
export function fingerprintCanvasOrientations(
  ...
  const inverseArtwork = oppositeRegion(CARD_REGIONS.artwork);
  return [
    fingerprintCanvas(source),
    createCompactFingerprintFromSamples(
      sampleCanvas(source, FULL_WIDTH, FULL_HEIGHT, undefined, true),
      sampleCanvas(source, ART_WIDTH, ART_HEIGHT, inverseArtwork, true),
      sampleCanvas(source, COLOUR_WIDTH, COLOUR_HEIGHT, inverseArtwork, true),
    ),
  ];
}
...
export function compareCompactCoarseDecoded(
  ...
  return clamp(artwork * 0.55 + fullCard * 0.25 + colour * 0.20);
}
